...
export default {
  ...
  watch: {
    ...
    adjustment_stocking_request_id: {
      async handler(newVal) {
        await this.loadAdjustmentStockingRequest(newVal)

        this.$nextTick(() => {
          this.populateForm(this.adjustmentStockingRequest)
        })
      },
      immediate: true,
    },
  ...
  methods: {
    populateForm(adjustmentRequest) {
      if (adjustmentRequest.branch?.id) {
      ...
      this.localFormData = {
        ...
        final_products: this.populateFinalProducts(adjustmentRequest),
      }

      this.$emit('appendProductToCart', this.localFormData.final_products)
    },

    populateFinalProducts(adjustmentRequest) {
      if (adjustmentRequest.final_products?.length) {
        return adjustmentRequest.final_products.map((adjustmentRequestProduct, index) => {
          return {
            ...
            quantity: adjustmentRequestProduct.quantity,
            measurement_unit_id: adjustmentRequestProduct.measurement_unit?.id,
          }
        })
      }
      return []
