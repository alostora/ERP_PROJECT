<template>
  <div class="page">
    <div class="page-header">
      <h1 class="page-title">{{ $t('paymentIncomeItems.title') }}</h1>
    ...
    <div class="card">
      ...
      <DataTable
        ...
        showGridlines
      >
        <Column field="id" :header="$t('paymentIncomeItems.id')" class="col-1">
          <template #body="slotProps">
            <span class="font-mono text-sm">{{ slotProps.index + 1 }}</span>
          </template>
        </Column>

        <Column field="name" :header="$t('paymentIncomeItems.name')" sortable />

        <Column field="name_ar" :header="$t('paymentIncomeItems.name_ar')" sortable />
...
export default {
    ...
    '$route.params.company_id': {
      handler(newVal) {
        if (newVal && newVal !== 'undefined') {
          ...
          this.apiUrl = `${API_ROUTES.PAYMENT_INCOME_ITEM.SEARCH}/${newVal}`
        }
      },
      immediate: true,
    },
  ...
  data() {
    return {
      apiUrl: API_ROUTES.PAYMENT_INCOME_ITEM.SEARCH,
      deleteUrl: API_ROUTES.PAYMENT_INCOME_ITEM.BASE,
      company_id: '',
