...
export default {
  name: 'UpdateToWarehouseStageForm',
  ...
  methods: {
    populateForm(selectedItem) {
      this.stage_sort = selectedItem.to_warehouse_stage?.stage_sort
      this.activeStep = String(selectedItem.to_warehouse_stage?.stage_sort)
      this.formData = {
        id: selectedItem.id || '',
        to_warehouse_stage_id: selectedItem.to_warehouse_stage?.id || '',
      }
    },
    ...
    goToStep(stage, parent = null) {
      ...
      if (parent == null) {
        if (stage.stage_sort > this.minStageSort) {
          ...
          this.formData.to_warehouse_stage_id = backStage.id
        }

        if (stage.stage_sort < this.maxStageSort) {
          const nextStage = this.stages.find((s) => s.stage_sort === stage.stage_sort + 1)
          this.formData.to_warehouse_stage_id = nextStage.id
        }
      } else {
        this.formData.to_warehouse_stage_id = stage.id
      }
    ...
    async handleSubmit() {
      if (!this.validateUpdateToStageForm(this.formData)) {
        return
      }
