<template>
  <Dialog
    ...
    :header="$t('common.createTitle', { module: $t('transportRequests.title') })"
    modal
            ...
            <Card>
              ...
              <template #content>
                <form @submit.prevent="handleSubmit" @click.stop v-if="!selected_item.is_closed">
                  <label class="form-label">{{ $t('transportRequests.stage_comment') }}</label>
                  <textarea
                    v-model="formData.comment"
                    class="textarea w-full h-full"
                    rows="3"
                    :placeholder="$t('transportRequests.stage_comment')"
                    @click.stop
                  ></textarea>
                </form>
...
export default {
  name: 'UpdateFromWarehouseStageForm',
  ...
  data() {
    return {
      apiUrl: API_ROUTES.TRANSPORT_REQUEST.UPDATE_STAGE_FROM_WAREHOUSE,
      activeStep: String(this.stage_sort),
      stage_sort: '',
      formData: {
        id: '',
        company_id: this.company_id,
        from_warehouse_stage_id: '',
        comment: '',
      },
    }
  },
  ...
  methods: {
    populateForm(selectedItem) {
      this.stage_sort = selectedItem.from_warehouse_stage?.stage_sort
      this.activeStep = String(selectedItem.from_warehouse_stage?.stage_sort)
      this.formData = {
        id: selectedItem.id || '',
        from_warehouse_stage_id: selectedItem.from_warehouse_stage?.id || '',
      }
    },
    ...
    goToStep(stage, parent = null) {
      ...
      if (parent == null) {
        if (stage.stage_sort > this.minStageSort) {
          ...
          this.formData.from_warehouse_stage_id = backStage.id
        }

        if (stage.stage_sort < this.maxStageSort) {
          const nextStage = this.stages.find((s) => s.stage_sort === stage.stage_sort + 1)
          this.formData.from_warehouse_stage_id = nextStage.id
        }
      } else {
        this.formData.from_warehouse_stage_id = stage.id
      }
    ...
    async handleSubmit() {
      if (!this.validateUpdateFromStageForm(this.formData)) {
        return
      }
