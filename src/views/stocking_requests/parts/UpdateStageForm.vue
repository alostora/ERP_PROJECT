            ...
            <Card>
              ...
              <template #content>
                <form @submit.prevent="handleSubmit" @click.stop v-if="!selected_item.is_closed">
                  <label class="form-label">{{ $t('salesInvoices.stage_comment') }}</label>
                  <textarea
                    v-model="formData.comment"
                    class="textarea w-full h-full"
                    rows="3"
                    :placeholder="$t('common.add') + ' ' + $t('salesInvoices.stage_comment')"
                    @click.stop
                  ></textarea>
                </form>
...
export default {
  ...
  data() {
    return {
      apiUrl: API_ROUTES.STOCKING_REQUEST.UPDATE_STAGE,
