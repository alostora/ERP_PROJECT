    ...
    <div class="card">
      <div class="flex align-center gap-3 mb-2">
        <button
          ...
          @click="$router.push(`/company/branch/${company_id}/${branch_id}`)"
        >
      ...
      <DataTable
        ...
        showGridlines
      >
        <Column field="id" :header="$t('warehouses.id')" class="col-1">
        ...
        <Column field="name_ar" :header="$t('warehouses.name_ar')" sortable />

        <Column :header="$t('warehouses.is_default')">
          <template #body="{ data }">
            <div v-if="data.is_default" class="badge badge-success">
              {{ $t('common.yes') }}
            </div>
            <div v-else-if="!data.is_default">
              <ToggleSwitch v-model="data.is_default" @change="setDefaultWarehouse(data.id)" />
            </div>
          </template>
        </Column>
...
export default {
    ...
    '$route.params.company_id': {
      handler(newVal) {
        if (newVal && newVal !== 'undefined') {
          this.apiUrl = `${API_ROUTES.WAREHOUSE.SEARCH}/${newVal}`
        }
      },
      immediate: true,
    },
  ...
  data() {
    return {
      apiUrl: API_ROUTES.WAREHOUSE.SEARCH,
      deleteUrl: API_ROUTES.WAREHOUSE.BASE,
      filters: { query_string: '', branch_id: this.branch_id },
      selectedItem: {},
    }
  },
