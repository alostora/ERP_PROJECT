<template>
  <div class="page">
    <div class="page-header">
      <h1 class="page-title">{{ $t('contacts.emails') }}</h1>
        ...
        <Column :header="$t('contacts.is_default')">
          <template #body="{ data }">
            ...
            <div v-else-if="!data.is_default">
              <ToggleSwitch v-model="data.is_default" @change="setDefaultEmail(data.id)" />
            </div>
          </template>
        </Column>
...
export default {
  ...
  data() {
    return {
      apiUrl: API_ROUTES.CONTACT_EMAIL.SEARCH,
      deleteUrl: API_ROUTES.CONTACT_EMAIL.BASE,
