<template>
  <div class="page">
    <div class="page-header">
      <h1 class="page-title">{{ $t('contacts.phones') }}</h1>
        ...
        <Column :header="$t('contacts.is_default')">
          <template #body="{ data }">
            ...
            <div v-else-if="!data.is_default">
              <ToggleSwitch v-model="data.is_default" @change="setDefaultPhone(data.id)" />
            </div>
          </template>
        </Column>
...
export default {
  ...
  watch: {
    contact_id: {
      handler(newVal) {
        if (newVal) {
          this.apiUrl = `${API_ROUTES.CONTACT_PHONE.SEARCH}/${newVal}`
  ...
  data() {
    return {
      apiUrl: API_ROUTES.CONTACT_PHONE.SEARCH,
      deleteUrl: API_ROUTES.CONTACT_PHONE.BASE,
